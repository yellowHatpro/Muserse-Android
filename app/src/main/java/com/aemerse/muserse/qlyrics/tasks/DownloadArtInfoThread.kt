        ...
        private fun getRunnable(
            ...
            return object : Runnable {
                ...
                private fun getImageUrl(): String {
                    ...
                    var access_token = ""
                    try {
                        val time: Long = ApplicationClass.getPref().getLong("spoty_expiry_time", 0)
                        val diff: Long = System.currentTimeMillis() - time
                        Log.d("ArtInfoThread", "getImageUrl: difference $diff")
                        val i = if (diff / 1000 < 3600) {
                            access_token = ApplicationClass.getPref().getString("spoty_token", "")!!
                            Log.d("ArtInfoThread",
                            ...
                            if (response != null && response.has("access_token")) {
                                ...
                                ApplicationClass.getPref().edit()
                                    ...
                                    .commit()
                                ApplicationClass.getPref().edit().putString("spoty_token", token)
                                    .commit()
                                access_token = token
                            }
                            Log.d("ArtInfoThread",
