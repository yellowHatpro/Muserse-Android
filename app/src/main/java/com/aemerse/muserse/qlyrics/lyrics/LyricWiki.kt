...
import com.aemerse.muserse.qlyrics.lyricsAndArtistInfo.Net.getUrlAsString
...
import com.aemerse.muserse.qlyrics.lyrics.Lyrics.Companion.NO_RESULT
import com.aemerse.muserse.qlyrics.lyrics.Lyrics.Companion.POSITIVE_RESULT
import com.aemerse.muserse.qlyrics.lyrics.Lyrics.Companion.SEARCH_ITEM
...
object LyricWiki {
    ...
    private val baseUrl = "http://lyrics.wikia.com/api.php?action=lyrics&fmt=json&func=getSong&artist=%1s&song=%1s"
    private val baseAPIUrl = "http://lyrics.wikia.com/wikia.php?controller=LyricsApi&method=getSong&artist=%1s&song=%2s"
    private val baseSearchUrl = "http://lyrics.wikia.com/Special:Search?search=%s&fulltext=Search"

    @Reflection
    fun search(query: String): ArrayList<Lyrics> {
        var query: String = query
        val results: ArrayList<Lyrics> = ArrayList()
        query = "$query song"
        query = Normalizer.normalize(query, Normalizer.Form.NFD)
            .replace("\\p{InCombiningDiacriticalMarks}+".toRegex(), "")
        try {
            val queryURL = URL(String.format(baseSearchUrl, URLEncoder.encode(query, "UTF-8")))
    ...
    fun fromURL(url: String, artist: String?, song: String?): Lyrics {
        ...
        try {
            ...
            val title: String = lyricsPage.getElementsByTag("title")[0].text()
            val colon: Int = title.indexOf(':')
            if (artist == null) artist = title.substring(0, colon).trim()
            if (song == null) {
