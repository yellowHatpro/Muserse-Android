...
import com.aemerse.muserse.BuildConfig
...
import com.aemerse.muserse.qlyrics.tasks.BulkArtInfoGrabber
...
class MusicLibrary private constructor() {
    private val context: Context = ApplicationClass.getContext()
    ...
    fun RefreshLibrary() {
        val excludedFoldersString = ApplicationClass.getPref().getString(context.getString(R.string.pref_excluded_folders), "")
        excludedFolders = excludedFoldersString!!.split(",".toRegex()).toTypedArray()

        //filter audio based on track duration
        SHORT_CLIPS_TIME_IN_MS = ApplicationClass.getPref().getInt(context.getString(R.string.pref_hide_short_clips), 10) * 1000

        //filter audio based on name
        REMOVE_TRACK_CONTAINING_1 = ApplicationClass.getPref().getString(context.getString(R.string.pref_hide_tracks_starting_with_1), "")
        REMOVE_TRACK_CONTAINING_2 = ApplicationClass.getPref().getString(context.getString(R.string.pref_hide_tracks_starting_with_2), "")
        REMOVE_TRACK_CONTAINING_3 = ApplicationClass.getPref().getString(context.getString(R.string.pref_hide_tracks_starting_with_3), "")
        atomicInt.set(0)
    ...
    private fun fillDataForArtist() {
        Executors.newSingleThreadExecutor().execute {
            ...
            if (!BuildConfig.DEBUG) {
                ...
                val lastTimeDidAt = ApplicationClass.getPref()
    ...
    fun getAlbumArtUri(album_id: Int): Uri? {
        return Uri.fromFile(File(ApplicationClass.getInstance().filesDir.absolutePath + "random.png"))
    ...
    fun getAlbumArtFromTrack(trackId: Int): Bitmap? {
        ...
        var bm: Bitmap? = null
        try {
            bm = ApplicationClass.getContext().contentResolver.loadThumbnail(trackUri, Size(512, 512), null)
        } catch (e: IOException) {
            e.printStackTrace()
        }
        return bm
